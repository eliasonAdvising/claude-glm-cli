// Refactored implementation with three named helpers.
...
export function normalizeAccount(raw) {
  const sign = Math.sign(raw.balance);
  const rounded = sign * Math.round(Math.abs(raw.balance) * 100) / 100;
  return {
    name: raw.name.trim(),
    balance: rounded,
...
// The original entry point — keeps the same signature + behavior.
